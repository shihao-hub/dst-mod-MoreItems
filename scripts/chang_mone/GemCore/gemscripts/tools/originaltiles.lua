...
if CurrentRelease.GreaterOrEqualTo("R22_PIRATEMONKEYS") then
    local OriginalTileGrid

    local function CreateOriginalTileMap(map)
        local index = 0
        for tileid in BinaryString(map.tiles):LoopValues(9, nil, nil, 2) do
            OriginalTileGrid:SetDataAtIndex(index, tileid)
            index = index + 1
        end

        map.topology.original_world_tiles = OriginalTileGrid:Save()
    end

    if rawget(_G, "Map") then
        function Map:GetOriginalTile(x, y)
            if OriginalTileGrid == nil then
                return GROUND.IMPASSABLE
            end
            return OriginalTileGrid:GetDataAtPoint(x, y) or GROUND.INVALID
        end
    ...
    gemrun("ondoinitgame", function(callback, savedata, ...)
        OriginalTileGrid = DataGrid(savedata.map.width, savedata.map.height)
        if not savedata.map.topology.original_world_tiles then
            CreateOriginalTileMap(savedata.map)
        else
            OriginalTileGrid:Load(savedata.map.topology.original_world_tiles)
        end
        callback(savedata, ...)
    end)

    gemrun("ontileconversion", function(savedata, conversionmap)
        local original_world_tiles = savedata.map.topology.original_world_tiles
        for index, tileid in pairs(original_world_tiles) do
            original_world_tiles[index] = conversionmap[tileid] or tileid
        end
    end)
else
    local function CreateOriginalTileMap(map)
