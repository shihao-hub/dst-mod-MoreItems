...
local IngredientSet = Class(function(self, data, ingredientmod, validonly)
    ...
    if self:GetIngredient().amount > 0 or validonly then
...
local IngredientAllocation = Class(function(self, ingredientallocator, ingredient_priority)
    ...
    self.amountneeded = math.max(1, self:GetIngredientSet().amount)
    self.willconsume = {}
...
function IngredientAllocator:GetIngredientSets(ingredientmod, validonly)
    local ingredient_sets = {}
    for i, ingredient in ipairs(self.recipe.ingredients) do
        local found_items = {} --GRUMBLE GRUMBLE this is needed due to a weird way taking half of a stack works when its predicted on the client.
        local items = self.builder.replica.inventory:FindCraftingItems(function(item)
            if not found_items[item] then
        ...
        if count >= math.max(1, math.round(ingredient.amount * ingredientmod)) or self.allowinvalid then
            table.insert(ingredient_sets, IngredientSet({ingredient = ingredient, itemoptions = itemoptions}, ingredientmod, validonly))
        end
    end
    for i, gemdict_ingredient in ipairs(self.recipe.gemdict_ingredients) do
        ...
        if count >= math.max(1, math.round(gemdict_ingredient.amount * ingredientmod)) or self.allowinvalid then
            if gemdict_ingredient.allowmultipleprefabtypes then
                table.insert(ingredient_sets, IngredientSet({gemdict_ingredient = gemdict_ingredient, itemoptions = itemoptions}, ingredientmod, validonly))
            else
                local itemcounts = {}
                ...
                if #_itemoptions > 0 or self.allowinvalid then
                    table.insert(ingredient_sets, IngredientSet({gemdict_ingredient = gemdict_ingredient, canmix = false, itemoptions = _itemoptions}, ingredientmod, validonly))
                end
            end
        end
    end
...
function IngredientAllocator:GetBuilderItems()
    ...
    for ingredient_priority, ingredient_allocation in ipairs(self.ingredient_allocations) do
        local real_amount = ingredient_allocation:GetIngredientSet().amount
...
function IngredientAllocator:GetRecipeIngredients(builder, ingredientmod, validonly)
    ...
    self.ingredient_sets = self:GetIngredientSets(ingredientmod, validonly)
