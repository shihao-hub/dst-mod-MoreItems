...
local BinaryString = Class(function(self, binarypackedstring, debugoutput)
    ...
    function self:LoopValues(start, startlimit, endlimit, valuesize, extraskip, signed)
        local current = BinaryPosition(start or 0, binarydata)
        startlimit = math.max(startlimit or 0, 0)
        endlimit = math.min(endlimit or size, size)
        valuesize = valuesize or 1
        extraskip = extraskip or 0

        return coroutine.wrap(function()
            while startlimit <= current.position and current.position <= endlimit do
                local value = ValueReadBytes(valuesize, signed, current, false)
                local function writevaluefunction(val)
                    ValueWriteBytes(valuesize, val, current, false)
                end
                coroutine.yield(value, writevaluefunction)
                current = current + valuesize + extraskip
            end
        end)
    end
