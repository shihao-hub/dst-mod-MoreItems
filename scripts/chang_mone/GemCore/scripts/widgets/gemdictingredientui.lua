...
local GemDictIngredientUI = Class(IngredientUI, function(self, images, quantity, counts, names, owner, recipe_type, quant_text_scale, ingredient_recipe)
    assert(IsGemDictIngredient(recipe_type), "recipe_type for GemDictIngredientUI is not "..GEMDICT_INGREDIENT)
    IngredientUI._ctor(self, images[1].atlas, images[1].image, quantity, counts[1].on_hand, counts[1].has_enough, names[1], owner, recipe_type, quant_text_scale, ingredient_recipe)
    ...
    if #images > 1 then
        self.inst:DoPeriodicTask(1, function() self:UpdateImage() end)
    end
end)

function GemDictIngredientUI:UpdateImage()
