...
local function Get(fn, name, file)
    ...
    while true do
        ...
        if upvalue and type(upvalue) == "function" and not visit[upvalue] then
            ...
            if upupvalue then
                return TryToClose(name, upupvalue, level)
            end
        end
        i = i + 1
    end
...
local function Set(fn, name, set, file)
    ...
    while true do
        ...
        if upvalue and type(upvalue) == "function" and not visit[upvalue] then
            ...
            if upupvalue then
                return TryToClose(upupvalue, nil, level)
            end
        end
        i = i + 1
    end
...
local function FunctionTest(fn, file, test, source, listener)
    ...
    if test and type(test) == "function" and not test(data, source, listener) then
        return false
    end    --测试通过
    return true
end
