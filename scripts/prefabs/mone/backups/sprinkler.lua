...
local function TurnOn(inst)
    ...
    if not inst.waterSpray then
        inst.waterSpray = SpawnPrefab("mone_water_spray")
...
local function OnEntitySleep(inst)
    --inst.SoundEmitter:KillSound("firesuppressor_idle") -- chang
end
...
local function UpdateSpray(inst)
    ...
    for k, v in pairs(ents) do
        ...
        if v.components.moisturelistener and not (v.components.inventoryitem and v.components.inventoryitem.owner) then
            ...
            v.components.moisturelistener:AddMoisture(0.5)
            --[[
...
local function fn()
    ...
    -- ???
    inst:SetStateGraph("mone_SGsprinkler")
...
return Prefab("mone_sprinkler", fn, assets, prefabs),
MakePlacer("mone_sprinkler_placer", "firefighter_placement", "firefighter_placement", "idle", true, nil, nil, 1.55, nil, nil, nil, nil, nil, PlaceTestFn)